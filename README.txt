...
current standard.
We believe it might be useful for users who wish to write or modify their own 
software in order to make use of the correctly-delimited discontiguous 
collocations.
...
rather than a word.
Then, two pseudo nodes are created to reference the discontiguous parts (lines 
39 and 41), and an edge is created between the two pseudo nodes (lines 42 to 
45).
